...
const Referendums = () => {
  ...
  if (error)
    return (
      <div className="flex items-center p-6 mb-4 space-x-4 border-l-4 rounded-lg bg-onehalf-red/20 border-onehalf-red text-onehalf-red" role="alert">
        <AlertCircle size={32} />
        <span className="text-lg font-bold">Failed to load referendum data!</span>
      </div>
    );
  ...
  if (!data)
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-4">
        ...
        <p className="text-onehalf-gray">Loading referendums...</p>
      </div>
    );

  const contests = data.data.contests || [];
  const referendums = contests.filter(contest => contest.type === 'Referendum');

  if (referendums.length === 0) {
    return (
      <div className="mb-6 border rounded-xl overflow-hidden border-onehalf-gray shadow-lg bg-onehalf-dark">
        <div className="px-6 py-4 bg-onehalf-dark border-b border-onehalf-gray/30 text-onehalf-light flex items-center">
          <Info className="mr-3 text-onehalf-cyan" size={28} />
          <h3 className="text-xl font-bold">Referendums not found in database</h3>
        </div>
        <div className="p-8 text-center space-y-4">
          <p className="text-lg text-onehalf-light">
            If you are aware of an upcoming referendum, please contact your state's election office.
          </p>
          <a
  ...
  return (
    <div className="mb-6 overflow-hidden border rounded-lg shadow-lg border-onehalf-gray bg-onehalf-dark">
      ...
      >
        <span className="text-xl font-bold font-righteous">Upcoming Referendum(s)</span>
        ...
        <div className="p-0 overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-onehalf-blue/10 text-onehalf-blue uppercase text-sm font-bold tracking-wider">
              <tr>
                <th className="px-6 py-4 border-b border-onehalf-gray/20">Title / Subtitle</th>
                ...
                <tr key={idx} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-6">
                    <div className="flex items-start space-x-3">
                      <FileText className="text-onehalf-cyan shrink-0 mt-1" size={24} />
                      <div>
                        <div className="font-bold text-xl text-onehalf-light mb-2">{referendum.referendumTitle}</div>
                        <div className="text-onehalf-gray italic">{referendum.referendumSubtitle}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-6">
          ...
          <div className="px-6 py-4 bg-black/20 text-onehalf-gray text-sm italic">
            List of referendum info in your area
          </div>
        </div>
      )}
    </div>
