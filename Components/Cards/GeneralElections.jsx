...
const GeneralElections = () => {
  ...
  useEffect(() => {
    try {
      ...
      // Fallback to raw values if they exist but aren't valid JSON
  ...
  if (error)
    return (
      <div className="flex items-center p-6 mb-4 space-x-4 border-l-4 rounded-lg bg-onehalf-red/20 border-onehalf-red text-onehalf-red" role="alert">
        <AlertCircle size={32} />
        <span className="text-lg font-bold">Failed to load general election data!</span>
      </div>
    );
  ...
  if (!data)
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-4">
        ...
        <p className="text-onehalf-gray">Loading general elections...</p>
      </div>
    );

  const contests = data.data.contests || [];
  const generalElections = contests.filter(contest => contest.type === 'General');

  if (generalElections.length === 0) {
    return (
      <div className="mb-6 border rounded-xl overflow-hidden border-onehalf-yellow shadow-lg bg-onehalf-dark">
        <div className="px-6 py-4 bg-onehalf-yellow text-onehalf-dark flex items-center">
          <Info className="mr-3" size={28} />
          <h3 className="text-xl font-bold">General elections not found in database</h3>
        </div>
        <div className="p-8 text-center space-y-4">
          <p className="text-lg text-onehalf-light">
            If you are aware of an upcoming election, please contact your state's election office.
          </p>
          <a
  ...
  return (
    <div className="mb-6 overflow-hidden border rounded-lg shadow-lg border-onehalf-gray bg-onehalf-dark">
      ...
      >
        <span className="text-xl font-bold font-righteous">Upcoming General Election(s)</span>
